/**
 * Cascade Step Handlers
 *
 * Registers concrete engine calls into the cascade orchestrator.
 * Each handler runs a deterministic engine and returns the next event
 * to continue the cascade.
 *
 * ASCM MPC CASCADE:
 *   DEMAND_UPDATED → DRP → PRODUCTION_PLAN → SCHEDULING → MRP → COMPLETE
 *
 * Each step:
 *   1. Takes the previous step's output as payload
 *   2. Runs the deterministic engine
 *   3. Returns { nextEvent, payload } for the next step
 */
...
// ─── Step 1: DEMAND_UPDATED → Run DRP ──────────────────────────

cascade.registerStep(CASCADE_EVENTS.DEMAND_UPDATED, async (payload, context) => {
  ...
  for (const product of products) {
    ...
    for (const dc of dcCodes) {
      ...
      const demand = payload.demandOverrides?.[dc]?.[sku] || dcDemandForecast[dc]?.[sku];
  ...
  const totalExceptions = drpResults.reduce((s, r) => s + (r.exceptions?.length || 0), 0);

  return {
    nextEvent: CASCADE_EVENTS.DRP_REBALANCED,
    payload: {
      periods,
      drpResults,
      plantGrossReqs,
...
// ─── Step 2: DRP_REBALANCED → Run Production Plan ──────────────

cascade.registerStep(CASCADE_EVENTS.DRP_REBALANCED, async (payload, context) => {
  const { periods, plantGrossReqs } = payload;
  const plantPlans = {};
  ...
  return {
    nextEvent: CASCADE_EVENTS.PRODUCTION_PLAN_CHANGED,
    payload: { ...payload, plantPlans },
  };
});

// ─── Step 3: PRODUCTION_PLAN_CHANGED → Run Scheduling ──────────

cascade.registerStep(CASCADE_EVENTS.PRODUCTION_PLAN_CHANGED, async (payload, context) => {
  const { periods, plantGrossReqs, plantPlans } = payload;
  ...
  return {
    nextEvent: CASCADE_EVENTS.SCHEDULE_UPDATED,
    payload: { ...payload, scheduleResults },
  };
});

// ─── Step 4: SCHEDULE_UPDATED → Run MRP per plant ──────────────

cascade.registerStep(CASCADE_EVENTS.SCHEDULE_UPDATED, async (payload, context) => {
  const { periods, plantGrossReqs } = payload;
  const mrpResults = {};
  ...
  return {
    ...
    payload: { ...payload, mrpResults },
  };
});

// ─── Step 5: MRP_RUN_COMPLETE → Cascade complete ───────────────

cascade.registerStep(CASCADE_EVENTS.MRP_RUN_COMPLETE, async (payload, context) => {
  // Terminal step — cascade is done
  // In the future, this would trigger AI exception analysis

  const summary = {
    planRunId: context.planRunId,
    drpSkusPlanned: payload.drpResults?.length || 0,
    drpExceptions: payload.totalExceptions || 0,
    plantsScheduled: Object.keys(payload.scheduleResults || {}).length,
    mrpPlants: Object.keys(payload.mrpResults || {}).length,
    totalMrpExceptions: Object.values(payload.mrpResults || {}).reduce((s, r) => s + r.totalExceptions, 0),
    criticalMrpExceptions: Object.values(payload.mrpResults || {}).reduce((s, r) => s + r.criticalExceptions, 0),
  };

  return {
    nextEvent: null, // Terminal — cascade ends here
    payload: summary,
  };
});
...
/**
 * Run a full ASCM cascade: Demand → DRP → Prod Plan → Scheduling → MRP
 *
 * @param {Object} options
 * @param {Object} [options.demandOverrides] - Override demand by DC/SKU
 * @param {boolean} [options.isScenario] - Mark as scenario (what-if)
 * @returns {Promise<Object>} - { planRunId, queued, rejected? }
 */
export async function triggerFullCascade(options = {}) {
  return cascade.trigger(
    CASCADE_EVENTS.DEMAND_UPDATED,
    { demandOverrides: options.demandOverrides },
    { isScenario: options.isScenario },
  );
}
