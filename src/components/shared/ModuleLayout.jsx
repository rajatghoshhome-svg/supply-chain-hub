...
export default function ModuleLayout({ moduleContext, tabs, activeTab, onTabChange, children }) {
  ...
  return (
    <div style={{ background: T.bg, minHeight: 'calc(100vh - 54px)', fontFamily: 'Inter' }}>
      {/* Tab bar */}
      {tabs && tabs.length > 0 && (
        <div style={{ background: T.white, borderBottom: `1px solid ${T.border}`, padding: '0 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ display: 'flex' }}>
        {/* Main content */}
        <div style={{ flex: 1, maxWidth: showAgent ? 'calc(100% - 380px)' : '100%', transition: 'max-width 0.2s' }}>
          {children}
        </div>

        {/* Agent panel */}
        {showAgent && (
          <div style={{ width: 380, flexShrink: 0, borderLeft: `1px solid ${T.border}`, background: T.white, height: 'calc(100vh - 54px - 46px)', position: 'sticky', top: 100, display: 'flex', flexDirection: 'column' }}>
