...
export default function LeafletMap({ navigate }) {
  ...
  const init = () => {
    ...
    // Routes — plant-to-DC lanes
    // PLT-NSH → DC-ATL (critical)
    L.polyline([[36.16, -86.78], [33.75, -84.39]], { color: T.risk, weight: 2.5, opacity: 0.8, dashArray: '8,5' }).addTo(map);
    // PLT-PDX → DC-LAS (review)
    L.polyline([[45.52, -122.68], [36.17, -115.14]], { color: T.warn, weight: 2, opacity: 0.7, dashArray: '6,6' }).addTo(map);
    // PLT-ATX → DC-CHI (stable)
    L.polyline([[30.27, -97.74], [41.88, -87.63]], { color: T.inkLight, weight: 1.5, opacity: 0.5, dashArray: '4,6' }).addTo(map);
    ...
    L.marker([34.9, -85.5], { icon: li('ACT NOW · 24hr', T.risk) }).addTo(map);
    L.marker([40.8, -118.9], { icon: li('REVIEW', T.warn) }).addTo(map);
