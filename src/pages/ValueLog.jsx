...
function statusStyle(s) {
  if (s === 'approved') return { color: T.safe,     bg: T.safeBg, border: `${T.safe}40` };
...
export default function ValueLog() {
  ...
  const RP = { 'Whole Foods': 0, Kroger: 0, Costco: 0, Target: 0, Amazon: 0 };
  approved.forEach(r => {
  ...
  return (
    <div style={{ background: T.bg, minHeight: 'calc(100vh - 54px)', fontFamily: 'Inter' }}>
      {/* Header */}
      <div className="valuelog-header" style={{ background: T.white, borderBottom: `1px solid ${T.border}`, padding: '16px 52px' }}>
      ...
      <div className="valuelog-body" style={{ maxWidth: 1160, margin: '0 auto', padding: '28px 52px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        {/* Summary KPIs */}
        <div className="valuelog-kpi-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', background: T.white, border: `1px solid ${T.border}`, borderRadius: 10, overflow: 'hidden' }}>
          {[
        ...
        <div style={{ background: T.white, border: `1px solid ${T.border}`, borderRadius: 10, padding: '18px 24px' }}>
          ...
          <div className="valuelog-retailer-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        ...
        <div style={{ background: T.white, border: `1px solid ${T.border}`, borderRadius: 10 }}>
          ...
          <div className="valuelog-table-scroll">
            {LOG.map((r, i) => {
              const s = statusStyle(r.status);
              return (
                <div key={r.id} className="rh valuelog-row" style={{ padding: '13px 22px', borderBottom: i < LOG.length - 1 ? `1px solid ${T.border}` : 'none', display: 'grid', gridTemplateColumns: '110px 1fr 100px 120px 1fr 100px', gap: 14, alignItems: 'center' }}>
                  <div>
